...
function getCurrencyCode(): string {
  // TODO: Map currency numbers to actual codes (use row.Currency when implemented)
  return 'USD';
}
...
export async function getFeaturedProperties(
  ...
  const rows = ((data ?? []) as RpcSummerRentPropertyRow[]).slice(0, limit);
...
export async function getProperties(limit?: number): Promise<Property[]> {
  ...
  const raw = (data ?? []) as RpcSummerRentPropertyRow[];
...
export async function getTopRatedPropertiesForHero(
  ...
  const baseProperties = ((data ?? []) as RpcSummerRentPropertyRow[])
...
export async function getPropertyById(id: string): Promise<Property | null> {
  ...
  const rows = (data ?? []) as RpcSummerRentPropertyRow[];
  const row = rows[0];
  if (!row) {
    return null;
  }
...
export async function searchProperties(
  ...
  let rows = (data ?? []) as RpcSummerRentPropertyRow[];
...
export async function getFavoriteProperties(
  ...
  const ids = (data ?? []).map((row: { EstatePropertyId: string }) => row.EstatePropertyId);
  if (ids.length === 0) {
    return [];
  }

  const { data: rpcData, error: rpcError } = await supabase.rpc('get_public_summer_rent_properties', {
    p_min_price: null,
    p_max_price: null,
    p_min_bedrooms: null,
    p_min_guests: null,
  ...
  const byId = new Map(
    ((rpcData ?? []) as RpcSummerRentPropertyRow[]).map((row) => [row.EstatePropertyId, row]),
  );
