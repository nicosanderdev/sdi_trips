...
type LoginVariant = 'main' | 'alt';

type LoginProps = {
  variant?: LoginVariant;
};

const Login: React.FC<LoginProps> = ({ variant = 'main' }) => {
  ...
  const navigateAfterAuth = async (userId: string | null) => {
    if (variant === 'alt') {
      navigate('/');
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        ...
        await navigateAfterAuth(userId ?? null);
      }
    } catch (err) {
  ...
  const handleMFAVerify = async () => {
    ...
    if (fullCode.length !== 6) {
      setMfaError(t('mfa.verification.incompleteCode'));
      return;
    }
    ...
    try {
      ...
      if (error) {
        setMfaError(error.message || t('mfa.verification.invalidCodeFallback'));
      } else {
        setShowMFAVerification(false);
        await navigateAfterAuth(userId ?? null);
      }
    } catch (err) {
      setMfaError(t('mfa.verification.verifyFailed'));
    } finally {
  ...
  const shell = (
    <>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                ...
                <p className="text-xl text-charcoal">{t('auth.signInToAccount')}</p>
              </div>
              ...
              <Card variant="default" className="p-8">
                {(fromSignupState?.fromSignup || resendMessage || resendError) && (
                  <div className="mb-6">
                    ...
                    >
                      <p className="font-semibold">{t('auth.verifyEmailPrompt')}</p>
                      <p className="mt-1">{t('auth.verifyEmailDescription')}</p>
                    </div>
                    ...
                    {resendError && <p className="mt-2 text-xs text-red-600 font-medium">{resendError}</p>}
                  </div>
                )}
              ...
              <p className="text-center text-charcoal">
                ...
                <Link to="/register" className="text-gold hover:text-navy transition-colors font-semibold">
                  {t('auth.signUpHere')}
                </Link>
              </p>
            </div>
      ...
      <Modal isOpen={showMFAVerification} onClose={handleCancelMFA} title={t('mfa.verification.title')}>
        <div className="space-y-6">
          <div className="text-center">
            <p className="text-charcoal mb-4">{t('mfa.verification.description')}</p>
            <p className="text-sm text-gray-500">{t('mfa.verification.required')}</p>
          </div>

          <div>
            <label className="block text-sm font-medium text-navy mb-3 text-center">
              {t('mfa.verification.codeLabel')}
            </label>
          ...
          <div className="flex justify-end space-x-3">
            ...
            <Button onClick={handleMFAVerify} disabled={mfaCode.join('').length !== 6 || isMFAVerifying}>
              {isMFAVerifying ? t('mfa.verification.verifying') : t('mfa.verification.verify')}
            </Button>
          </div>
        </div>
      </Modal>
  ...
  if (variant === 'main') {
    return <Layout showFooter={false}>{shell}</Layout>;
  }
  return shell;
};
