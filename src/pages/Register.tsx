...
const Register: React.FC = () => {
  ...
  const validateForm = () => {
    ...
    if (!formData.password) {
      ...
      newErrors.password = t('auth.errors.passwordMin8Chars');
    ...
    if (!formData.agreeToTerms) {
      newErrors.agreeToTerms = t('auth.errors.mustAgreeToTerms');
    }
  ...
  const passwordRequirements = [
    { text: t('auth.passwordRequirements.atLeast8Chars'), met: formData.password.length >= 8 },
    { text: t('auth.passwordRequirements.oneUppercase'), met: /[A-Z]/.test(formData.password) },
    { text: t('auth.passwordRequirements.oneLowercase'), met: /[a-z]/.test(formData.password) },
            ...
            <div className="space-y-8">
              ...
              <div className="space-y-4">
                <h1 className="text-4xl md:text-5xl font-thin text-navy">
                  {(() => {
                    const text = t('auth.joinHolidayTrips');
                    const words = text.split(' ');
                    const lastWords = words.slice(-2).join(' ');
                    const firstWords = words.slice(0, -2).join(' ');
                    return (
                      <>
                        {firstWords} <span className="font-bold text-gold">{lastWords}</span>
                      </>
                    );
                  })()}
                </h1>
                ...
                <div className="mt-8">
                  <div className="relative">
                    ...
                    <div className="relative flex justify-center text-sm">
                      <span className="px-4 bg-white text-charcoal">{t('auth.orSignUpWith')}</span>
                    </div>
                  </div>
            ...
            <div className="hidden lg:block">
              <div className="relative">
                ...
                <div className="absolute top-6 left-6 right-6 bg-white/95 backdrop-blur-md rounded-2xl p-6 shadow-gold">
                  <h3 className="text-lg font-semibold text-navy mb-3">{t('auth.whyJoinUs')}</h3>
                  <ul className="space-y-2 text-sm text-charcoal">
                    <li className="flex items-center space-x-2">
                      <Check className="h-4 w-4 text-green-600" />
                      <span>{t('auth.accessToProperties')}</span>
                    </li>
                    <li className="flex items-center space-x-2">
                      <Check className="h-4 w-4 text-green-600" />
                      <span>{t('auth.customerSupport')}</span>
                    </li>
                    <li className="flex items-center space-x-2">
                      <Check className="h-4 w-4 text-green-600" />
                      <span>{t('auth.exclusiveDeals')}</span>
                    </li>
                  </ul>
                </div>
              </div>
